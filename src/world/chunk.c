...
void chunk_init(chunk_t *self) {
    ...
    flist_init(&tiles, sizeof(tile_t), capacity);
    ...
    for (short i = 0; i < 9; i++) {
        ...
        ptr = flist_request(&tiles, CHUNK_MAX);
        page->tiles = array_init(ptr, sizeof(tile_t), CHUNK_MAX);
