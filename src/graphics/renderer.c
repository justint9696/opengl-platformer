...
void draw_quad(vec2s pos, vec2s dim, uint32_t color) {
    ...
    model = glms_translate(model, glms_vec3_add(renderer.camera->pos,
                                                (vec3s) { pos.x, pos.y, 0.f }));
...
void draw_line(vec2s start, vec2s end, uint32_t color) {
    ...
    mat4s model = GLMS_MAT4_IDENTITY_INIT;
    model = glms_translate(
        model,
        glms_vec3_add(renderer.camera->pos, (vec3s) { start.x, start.y, 0.f }));
